...
public struct LocalFilePersistenceService: CarbonLogPersistenceService {
    let fileURL: URL
    let mapper: CarbonLogMapper

    public init(fileURL: URL, mapper: CarbonLogMapper) {
...
func singleFilePersistenceServiceFactory(format: FileFormat, fileURL: URL) -> LocalFilePersistenceService {
    switch format {
    case .CSV: return LocalFilePersistenceService(fileURL: fileURL, mapper: CsvMapper())
    }
}

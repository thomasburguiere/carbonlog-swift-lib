...
struct CarbonLogPersistenceServiceTests {
    ...
    func persistAndLoadLog() async throws {
        ...
        var service = singleFilePersistenceServiceFactory(
            format: .CSV,
            fileURL: tempOutFileURL
        )

        // when
        try await service.persist(log: log)
        service = singleFilePersistenceServiceFactory(
            format: .CSV,
            fileURL: tempOutFileURL
        )
    ...
    func persistUpdateToLogLog() async throws {
        // given
        let service = singleFilePersistenceServiceFactory(
            format: .CSV,
            fileURL: getInputFilePathURL(fileName: "test-input")
        )
        // SingleFilePersistenceService(csvURL: getInputFilePathURL(fileName: "test-input"))
    ...
    func handleEmptyCsv() async throws {
        let service = singleFilePersistenceServiceFactory(
            format: .CSV,
            fileURL: getInputFilePathURL(fileName: "test-empty")
        )
    ...
    func handleGarbageCsv() async throws {
        let csvService = singleFilePersistenceServiceFactory(
            format: .CSV,
            fileURL: getInputFilePathURL(fileName: "test-garbage")
        )
